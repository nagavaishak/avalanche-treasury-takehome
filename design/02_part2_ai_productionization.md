...
The full filled-in example is at [`part2_workflow/pipeline_spec.example.yaml`](../part2_workflow/pipeline_spec.example.yaml) — covers metadata, purpose, freshness, inputs/outputs, dependencies, DQ checks, utility reuse, deploy target, and audit fields.
...
Final step. The system commits to a branch, opens a PR via `gh`, attaches a structured description with three sections: **What I did** (artifacts generated), **Where I am uncertain** (flagged items requiring verification), and **Human review required** (mandatory checklist before merge).

A full example PR description is at [`part2_workflow/examples/sample_pr_description.md`](../part2_workflow/examples/sample_pr_description.md).
